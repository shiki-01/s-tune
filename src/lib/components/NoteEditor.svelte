<script lang="ts">
	...
	import { activeNoteEditorTool, type NoteEditorTool } from '$lib/note-tools';
	...
	let editorState = $state({ selectedNoteIds: [] as string[] });
	let bpm = $state(120);
	...
	function setTool(tool: NoteEditorTool) {
		activeNoteEditorTool.set(tool);
	}
	...
	function pitchCurveSemitoneDelta(n: NoteSegment, tSec: number): number {
		...
		const u = (tSec - t0) / dur;
		const center = n.pitchCenterOffset;
		const modHz = 5.5;
		const modAmp = Math.max(0, n.vibratoDepth);
		const mod = Math.sin(2 * Math.PI * modHz * (tSec - t0)) * modAmp;
		const drift = (Math.max(0, Math.min(1, u))) * n.pitchDrift;
	...
	type DragState =
		...
		| {
			...
			pointerId: number;
			anchorTime: number;
			snapshot: Map<string, { startTime: number; endTime: number; sourceStartTime: number; sourceEndTime: number }>;
		}
		| {
			...
			anchorTime: number;
			activeId: string;
			snapshot: { startTime: number; endTime: number; sourceStartTime: number; sourceEndTime: number };
		}
		| {
			mode: 'pitch' | 'pitch-center-fine';
			pointerId: number;
			anchorY: number;
			anchorSemitone: number;
			snapshot: Map<string, { baseSemitone: number; pitchCenterOffset: number }>;
		}
		| {
			mode: 'drift';
			pointerId: number;
	...
	function onPointerDown(e: PointerEvent) {
		...
		const tool = $activeNoteEditorTool;
		...
		// Pitch tool: vertical drag changes baseSemitone; Alt+drag changes pitchCenterOffset (fine)
		if (tool === 'pitch') {
			if (!hit) {
			...
			const snapMap = new Map<string, { baseSemitone: number; pitchCenterOffset: number }>();
			snapMap.set(hit.note.id, {
				baseSemitone: hit.note.baseSemitone,
				pitchCenterOffset: hit.note.pitchCenterOffset
			});
			drag = {
				mode: e.altKey ? 'pitch-center-fine' : 'pitch',
				pointerId: e.pointerId,
				anchorY: y,
				anchorSemitone: semi,
				snapshot: snapMap
		...
		// Timing tool: drag body = shift (start/end) only; drag edges = resize (also updates source slice)
		if (tool === 'timing') {
			if (!hit) {
				if (!multi) setSelected([]);
				return;
			...
			if (hit.part === 'left' || hit.part === 'right') {
				...
				drag = {
					...
					snapshot: {
						...
						sourceStartTime: active.sourceStartTime,
						sourceEndTime: active.sourceEndTime
					}
				};
				svgEl.setPointerCapture(e.pointerId);
				e.preventDefault();
			...
			const snapMap = new Map<
				string,
				{ startTime: number; endTime: number; sourceStartTime: number; sourceEndTime: number }
			>();
			const n = hit.note;
			snapMap.set(n.id, {
				startTime: n.startTime,
				endTime: n.endTime,
				sourceStartTime: n.sourceStartTime,
				sourceEndTime: n.sourceEndTime
			});
			drag = {
				mode: 'time-move',
				pointerId: e.pointerId,
		...
		// Drift tool: click+drag to change pitchDrift
		if (tool === 'drift') {
			if (!hit) {
				if (!multi) setSelected([]);
				return;
			...
			drag = {
				...
				activeId: active.id,
				anchorY: y,
				snapshot: { pitchDrift: active.pitchDrift }
			};
			svgEl.setPointerCapture(e.pointerId);
			e.preventDefault();
			return;
	...
	function onPointerMove(e: PointerEvent) {
		...
		if (currentDrag.mode === 'time-move') {
			...
			updateNotes((n) => {
				...
				return { ...n, startTime: newStart, endTime: newEnd };
			});
			return;
		}

		if (currentDrag.mode === 'pitch' || currentDrag.mode === 'pitch-center-fine') {
			if (currentDrag.mode === 'pitch') {
				const dSemi = Math.round(semi - currentDrag.anchorSemitone);
				updateNotes((n) => {
					const base = currentDrag.snapshot.get(n.id);
					if (!base) return n;
					const newBase = clamp(base.baseSemitone + dSemi, MIN_SEMITONE, MAX_SEMITONE);
					return { ...n, baseSemitone: newBase };
				});
				return;
			}

			// fine: 100px => 1 semitone (1px => 1 cent)
			const dy = currentDrag.anchorY - y;
			const dSemi = dy / 100;
			updateNotes((n) => {
				const base = currentDrag.snapshot.get(n.id);
				if (!base) return n;
				return { ...n, pitchCenterOffset: base.pitchCenterOffset + dSemi };
			});
			return;
		}

		if (currentDrag.mode === 'drift') {
			const dy = currentDrag.anchorY - y;
			const dSemi = dy / 120; // 120px => 1 semitone
			const next = clamp(currentDrag.snapshot.pitchDrift + dSemi, -2, 2);
			updateNotes((n) => (n.id === currentDrag.activeId ? { ...n, pitchDrift: next } : n));
			return;
		}
		...
		if (currentDrag.mode === 'resize-left' || currentDrag.mode === 'resize-right') {
			...
			if (currentDrag.mode === 'resize-left') {
				...
				const newSrcStart = clamp(base.sourceStartTime + dt, 0, base.sourceEndTime - MIN_NOTE_SEC);
				updateNotes((n) =>
					n.id === currentDrag.activeId
						? { ...n, startTime: newStart, sourceStartTime: newSrcStart }
						: n
				);
				return;
			}
			...
			const newSrcEnd = clamp(base.sourceEndTime + dt, base.sourceStartTime + MIN_NOTE_SEC, track.duration);
			updateNotes((n) =>
				n.id === currentDrag.activeId
					? { ...n, endTime: newEnd, sourceEndTime: newSrcEnd }
					: n
			);
			return;
		}
		...
		if (drag.mode === 'pen') {
			...
			if (endTime > startTime + MIN_NOTE_SEC) {
				const seg = createNoteSegment({
					...
					sourceSemitone: drag.baseSemitone,
...
<div class="w:100% h:100% flex flex:column gap:10px">
	<div class="flex flex:row gap:10px ai:center">
		...
		<button
			type="button"
			class={
				$activeNoteEditorTool === 'pitch'
					? 'p:6px|8px bg:#333 fg:white r:6px flex ai:center jc:center'
					: 'p:6px|8px b:2px|solid|#333 r:6px flex ai:center jc:center'
			}
			onclick={() => setTool('pitch')}
		>
			Pitch (4)
		</button>
		<button
			type="button"
			class={
				$activeNoteEditorTool === 'timing'
					? 'p:6px|8px bg:#333 fg:white r:6px flex ai:center jc:center'
					: 'p:6px|8px b:2px|solid|#333 r:6px flex ai:center jc:center'
			}
			onclick={() => setTool('timing')}
		>
			Timing (5)
		</button>
		<button
